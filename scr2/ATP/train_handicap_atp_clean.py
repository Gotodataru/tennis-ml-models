...
"""
Обучение регрессионной модели games_diff (handicap) для ATP на чистом датасете.
Использует отфильтрованные признаки (82 шт.).
"""
...
MODEL_DIR = 'ATP/HANDICAP/model/games_diff_atp'
...
print("\n=== ОСНОВНЫЕ МЕТРИКИ (ATP games_diff) ===")
print(f"MAE  = {mae:.3f} ± {mae_std:.3f}")
print(f"RMSE = {rmse:.3f}")
print(f"R²   = {r2:.4f}")

# MAE по диапазонам (можно адаптировать при желании)
...
# ========== КАЛИБРОВКА ВЕРОЯТНОСТИ ДЛЯ КОНКРЕТНОЙ ФОРЫ (опционально) ==========
# Например, для форы -2.5 можно будет потом вычислять вероятность P(games_diff > 2.5) или P(games_diff < -2.5)
# Калибратор пока не обучаем, так как для разных фор понадобятся разные вероятности.
# Сохраняем только модель и статистику остатков.
...
plt.subplot(2, 4, 7)
# Можно построить гистограмму предсказанных значений
...
plt.subplot(2, 4, 8)
# Гистограмма фактических значений
